...
const PrintContext = contextEnabled
  ? React.createContext({} as IPrintContextProps)
  : null;
...
export interface IReactToPrintProps {
  ...
  nonce?: string;
  /** DOCTYPE for the document rendered inside the iframe */
  docType?:
    | boolean
...
export default class ReactToPrint extends React.Component<IReactToPrintProps> {
  ...
  public handlePrint = () => {
    const { bodyClass, content, copyStyles, fonts, pageStyle, nonce, docType } =
      this.props;
    ...
    printWindow.onload = () => {
      ...
      if (domDoc) {
        ...
        if (!isText) {
          ...
          const srcCanvasEls = (
            contentNodes as HTMLCanvasElement
          ).querySelectorAll("canvas");
          for (
            let i = 0, canvasElsLen = canvasEls.length;
            i < canvasElsLen;
            ++i
          ) {
          ...
          for (let i = 0; i < renderComponentVideoNodes.length; i++) {
            ...
            if (videoPoster) {
              ...
              if (videoNode.readyState >= 2) {
                ...
                videoNode.onerror = () => {
                  console.log("Video with no poster error");
                  markLoaded.bind(null, videoNode, false)();
                };
                // videoNode.onemptied = () => { console.log('Video with no poster emptied'); markLoaded.bind(null, videoNode, false)(); }
                videoNode.onstalled = () => {
                  console.log("Video with no poster stalled");
                  markLoaded.bind(null, videoNode, false)();
                };
              }
            }
          ...
          const originalInputs = (contentNodes as HTMLElement).querySelectorAll(
            inputSelector,
          ); // eslint-disable-line max-len
          ...
          const originalCRs = (contentNodes as HTMLElement).querySelectorAll(
            checkedSelector,
          ); // eslint-disable-line max-len
          ...
          const originalSelects = (
            contentNodes as HTMLElement
          ).querySelectorAll(selectSelector); // eslint-disable-line max-len
